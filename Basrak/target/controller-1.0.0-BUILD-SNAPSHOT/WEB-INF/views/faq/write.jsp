...
<script src="https://cdn.ckeditor.com/4.15.1/standard/ckeditor.js"></script>  <!-- 위지윅 에디터 쓰기위해 -->
  <h1 class="text-center">FaQ 글쓰기</h1>
  <div class="container">
    <div class="row">
      <form class="form-horizontal" action="/faq/insert" method="POST">
      <input type="hidden" value="${faq.f_seq}" name="f_seq" />  <!-- 데이터(번호)를 숨겨서 가져온다 -->
        ...
        <div class="form-group"> 
          ...
          <textarea class="form-control" rows="10" required="required" name="content"></textarea> <!-- 수정하기 누르면 전에 있는 글 내용을 불러온다 -->
          </div>
        </div>
                
        <div class="form-group">
          <div class="col-sm-offset-2 col-sm-10">
            <button type="submit" class="btn btn-success" id='btnSubmit'>등록하기</button> <!-- 게시판 등록하기 버튼 -->
            <button type="reset" class="btn btn-warning">취소하기</button>
            <button type="button" id="btnBack" onclick="javascript:fn_historyBack();" class="btn btn-info">돌아가기</button> 
          </div>
        </div>
      </form>
    </div>
